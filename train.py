...
def main(args):
    ...
    trainset = Dataset(data["train"], args)
    devset = Dataset(data["dev"], args)
    testset = Dataset(data["test"], args)

    log.debug("Building model...")
    
    model_file = args.data_root + "/model_checkpoints/model.pt"
    model = CORECT(args).to(args.device)
    opt = Optimizer(args.learning_rate, args.max_grad_value, args.weight_decay)
